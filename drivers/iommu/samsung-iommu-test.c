...
/* Common registers */
#define MMU_STATUS		0x08
#define MMU_VERSION		0x34
...
#define MMU_V7_4_EMU_PRELOAD	0x3040
#define MMU_V7_4_EMU_SHOT	0x3044
#define MMU_V7_2_EMU_PRELOAD	0x8040
#define MMU_V7_2_EMU_SHOT	0x8044
...
struct iommu_test {
	...
	unsigned int version;			/* SysMMU version */
};
...
static void iommu_test_do_translation(const struct iommu_test *obj)
{
	...
	const u32 pid	= 0x0;
	const u32 sid	= 0x2;
	u32 emu_preload, emu_shot;

	pr_info("initiate IOMMU translation (\"traffic emulation\")\n");

	/* Figure out EMU register offsets for this SysMMU version */
	if (MMU_MIN_VER(obj->version) < 4) {
		emu_preload = MMU_V7_2_EMU_PRELOAD;
		emu_shot = MMU_V7_2_EMU_SHOT;
	} else {
		emu_preload = MMU_V7_4_EMU_PRELOAD;
		emu_shot = MMU_V7_4_EMU_SHOT;
	}
	...
	iowrite32(rw | vpn, obj->reg_base + emu_preload);

	/* Issue emulation (initiate the translation) */
	iowrite32(sid | (pid << SHOT_PID_OFFSET), obj->reg_base + emu_shot);
}
...
static int iommu_test_probe(struct platform_device *pdev)
{
	...
	drv->version = readl(drv->reg_base + MMU_VERSION);
	drv->version = MMU_RAW_VER(drv->version);
	dev_info(drv->dev, "SysMMU version: %d.%d\n",
		 MMU_MAJ_VER(drv->version), MMU_MIN_VER(drv->version));
	if (MMU_MAJ_VER(drv->version) < 7) {
		dev_err(drv->dev, "Error: Only ver 7+ is supported\n");
		ret = -EINVAL;
		goto err_invalid_ver;
	}
